...
def label(checkpoint_path, csv_path):
    ...
    device = 'cpu'
    if torch.cuda.device_count() > 1: #works even if only 1 GPU available
...
# source ~/venvs/qwen3vl/bin/activate
# python src/label.py -d=src/test_report.csv -o=src/out -c=src/chexbert.pth