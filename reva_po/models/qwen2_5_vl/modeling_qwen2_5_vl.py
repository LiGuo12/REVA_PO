...
def apply_rotary_pos_emb_vision(tensor: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    '''
    tensor: [batch_size, num_heads, seq_length, head_dim]
    freqs: [batch_size, seq_length, rot_dim]
    '''
    ...
    cos = cos.unsqueeze(1).repeat(1, 1, 1, 2).float()  # B, 1, seq, head_dim
    sin = sin.unsqueeze(1).repeat(1, 1, 1, 2).float()  # B, 1, seq, head_dim
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        seq_length = hidden_states.shape[0] // batch_size
        
        # Reshape to batch format for attention computation
        hidden_states = hidden_states.reshape(batch_size, seq_length, -1)
        q, k, v = self.qkv(hidden_states).reshape(batch_size, seq_length, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4).unbind(0)
        
        
        # Apply rotary embeddings
        q = apply_rotary_pos_emb_vision(q, rotary_pos_emb.reshape(batch_size, seq_length, -1)) # q: [batch_size, num_heads, seq_length, head_dim]
        k = apply_rotary_pos_emb_vision(k, rotary_pos_emb.reshape(batch_size, seq_length, -1)) # rotary_pos_emb: [batch_size*seq_length, rot_dim] -> [batch_size, seq_length, rot_dim]
        
        # Create attention mask for batched computation
        attention_mask = torch.ones([batch_size, 1, seq_length, seq_length], device=q.device, dtype=torch.bool)
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.patch_embed(hidden_states) # [batch_size * seq_len, hidden_size]
        ...
        seq_length_sum, hidden_dim = hidden_states.shape
        ...
        # hidden_states_retrieval = hidden_states.clone().detach()
        
        hidden_states = self.merger(hidden_states)
        reverse_indices = torch.argsort(window_index)
        hidden_states = hidden_states[reverse_indices, :]

        # hidden_states_retrieval = hidden_states_retrieval[reverse_indices, :]
        
        # return hidden_states, hidden_states_retrieval
        return hidden_states
...
class Qwen2_5_VLCausalLMOutputWithPast(ModelOutput):
    ...
    inputs_embeds: Optional[torch.FloatTensor] = None
    image_embeds_retrieval: Optional[torch.FloatTensor] = None
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        second_per_grid_ts: Optional[torch.Tensor] = None,
        only_extract_image: Optional[bool] = None,
        image_embeds: Optional[torch.FloatTensor] = None,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.model.embed_tokens(input_ids)
            # original_inputs_embeds = self.model.embed_tokens(input_ids).detach()

            if pixel_values is not None:
                if image_embeds is None:
                    pixel_values = pixel_values.type(self.visual.dtype)
                    # image_embeds, image_embeds_retrieval = self.visual(pixel_values, grid_thw=image_grid_thw)
                    image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)
                    n_image_tokens = (input_ids == self.config.image_token_id).sum().item()
                    n_image_features = image_embeds.shape[0]
                    if n_image_tokens != n_image_features:
        ...
        return Qwen2_5_VLCausalLMOutputWithPast(
            ...
            image_embeds= image_embeds,
            # image_embeds_retrieval= image_embeds_retrieval,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Get retrieved embeddings from kwargs
        # retrieved_image_embeds = kwargs.get("retrieved_image_embeds", None)
        ...
        model_inputs.update(
            {
                ...
                # "retrieved_image_embeds": retrieved_image_embeds,
            }
        )
        return model_inputs
