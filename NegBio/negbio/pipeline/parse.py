...
class Bllip(object):
    def __init__(self, model_dir=None):
        ...
        # ------- Silent caching mechanism -------
        global _RERANKING_PARSER_CACHE
        if self.model_dir not in _RERANKING_PARSER_CACHE:
            logging.debug('loading model %s ...' % self.model_dir)
            _RERANKING_PARSER_CACHE[self.model_dir] = RerankingParser.from_unified_model_dir(self.model_dir)
        else:
            logging.debug('reusing cached parser for %s ...' % self.model_dir)
        self.rrp = _RERANKING_PARSER_CACHE[self.model_dir]
        # logging.debug('loading model %s ...' % self.model_dir)
