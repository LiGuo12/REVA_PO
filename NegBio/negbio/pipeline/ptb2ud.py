            ...
            for sentence in passage.sentences:
                ...
                try:
                    ...
                    if dependency_graph is None:
                        logging.error(
                            f"Dependency graph is None for sentence {sentence.offset} in {document.id}: {sentence.text}"
                        )
                        continue
                    res = convert_dg(dependency_graph, sentence.text,
                                            sentence.offset,
                                            has_lemmas=self._backend == 'jpype')
                    if res is None:
                        # logging.error(
                        #     f"convert_dg returned None for sentence {sentence.offset} in {document.id}: {sentence.text}"
                        # )
                        continue
                    anns, rels = res
                    ...
                    logging.exception(f"Cannot process sentence {sentence.offset} in {document.id}: {sentence.text}")
                    logging.error(f"Parse tree: {sentence.infons.get('parse tree')}")
                if self._backend != 'jpype':
