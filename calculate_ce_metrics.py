...
def evaluate_pred_vs_gt(
    ...
    if merged.empty:
        raise ValueError("对齐后为空，无法评估。（请检查键是否一致）")
...
def parse_args():
    ...
    parser.add_argument(
        "--exp_name",
        type=str,
        default="APRPO_stage3_MIMIC",
        help="Experiment folder name under <repo_root>/aprpo/output/.",
    )
    parser.add_argument(
        "--run_ts",
        type=str,
...
def main():
    ...
    # Repo root: this script is assumed to be located directly under APRPO/
    REPO_ROOT = Path(__file__).resolve().parent

    # Experiment root containing run subfolders named by timestamps (or similar)
    EXP_ROOT = REPO_ROOT / "aprpo" / "output" / args.exp_name
